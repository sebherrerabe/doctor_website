...
const apiHost = process.env.DJANGO_API_HOST;
...
export const getNewsList = async ({ page, env }: { page?: number; env?: "server" | "client" } = {}) => {
  const { data: news } = await axios.get<IPagination<INews>>(
    `${env === "server" ? apiHost : ""}/api/news/${page ? `?page=${page}` : ""}`
  );
  return news;
};
